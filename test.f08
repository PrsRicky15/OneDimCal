...
contains
    ...
    call grid%genPoten(harm_pot, v_array)
    call grid%printPotToFile(harm_pot, "harmonic.dat")

    ! Test Morse Potential: V = D * (1 - exp(-a*(x-re)))^2
    morse_pot = create_morse(D=5.0_dp, a=1.5_dp, re=2.0_dp)
    call grid%genPoten(morse_pot, v_array)
    call grid%printPotToFile(morse_pot, "morse_potential.dat")
    ...
    call grid%genPoten(multi_gauss_pot, v_array)
    call grid%printPotToFile(multi_gauss_pot, "multi_gaussian.dat")

    ! Test Super-Gaussian
    super_pot = create_SuperGauss(amplitude=1.5_dp, sigma=0.8_dp, &
            x0=0.0_dp, norder=4)
    call grid%genPoten(super_pot, v_array)
    call grid%printPotToFile(super_pot, "super_gaussian.dat")
    ...
    call grid%genPoten(super_multi_pot, v_array)
    call grid%printPotToFile(super_multi_pot, "super_multiGaussian.dat")

    ! Test Quartic Polynomial: V = c0 + c2*(x-x0)^2 + c4*(x-x0)^4
    poly_pot = create_quartic(x0=0.0_dp, c0=0.0_dp, c2=1.0_dp, c4=0.1_dp)
    call grid%genPoten(poly_pot, v_array)
    call grid%printPotToFile(poly_pot, "quadratic_poten.dat")
    ...
    call grid%genPoten(poly_pot, v_array)
    call grid%printPotToFile(poly_pot, "polynomial_poten.dat")
